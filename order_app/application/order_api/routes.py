...
def order_add_item():
    ...
    api_key = "$5$rounds=535000$2S5AIZ14CR7wN8Rg$CpT6bx7j0ty6K4o7ucg7Seo1KNeRgURKMtK3zN8ErC1"
    response = UserClient.get_users(api_key)
...
def order():
    ...
    if open_order is None:
        response = jsonify({"message": "Not order found"})
    else:
...
def checkout():
    ...
    order_model = Order.query.filter_by(user_id=user["id"], is_oped=1).first()
